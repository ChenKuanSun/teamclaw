...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    MockCognitoUserPool.prototype.getCurrentUser = jest
      .fn()
  ...
  describe('initialization', () => {
    ...
    it('should restore session from existing user on construction', () => {
      ...
      const mockUser = {
        getSession: jest.fn(
          (cb: (err: Error | null, session: CognitoUserSession) => void) => {
            cb(null, mockSession);
          },
        ),
  ...
  describe('login', () => {
    it('should authenticate user and update signals on success', async () => {
      ...
      MockCognitoUser.prototype.authenticateUser = jest.fn(
        (_authDetails, callbacks) => {
  ...
  describe('signOut', () => {
    it('should sign out and navigate to login', () => {
      ...
      const poolInstance =
        MockCognitoUserPool.mock.instances[
          MockCognitoUserPool.mock.instances.length - 1
        ];
      (poolInstance.getCurrentUser as jest.Mock).mockReturnValue(
        mockCurrentUser,
      );
  ...
  describe('forgotPassword', () => {
    it('should resolve when forgotPassword succeeds', async () => {
      MockCognitoUser.prototype.forgotPassword = jest.fn(function (
        this: CognitoUser,
      ...
      expect(MockCognitoUser.prototype.forgotPassword).toHaveBeenCalled();
    });

    it('should reject when forgotPassword fails', async () => {
  ...
  describe('confirmNewPassword', () => {
    it('should resolve when confirmPassword succeeds', async () => {
      MockCognitoUser.prototype.confirmPassword = jest.fn(function (
        this: CognitoUser,
        _code: string,
        _newPassword: string,
        callbacks: { onSuccess: () => void; onFailure: (err: Error) => void },
      ...
      const service = createService();
      await expect(
        service.confirmNewPassword('user@test.com', '123456', 'newpass'),
      ).resolves.toBeUndefined();
      expect(MockCognitoUser.prototype.confirmPassword).toHaveBeenCalledWith(
        '123456',
        'newpass',
    ...
    it('should reject when confirmPassword fails with wrong code', async () => {
      ...
      const service = createService();
      await expect(
        service.confirmNewPassword('user@test.com', '000000', 'newpass'),
