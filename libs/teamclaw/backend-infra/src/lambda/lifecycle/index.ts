...
import { getEnvVarPrefix } from '../admin/integrations/catalog-seed';
...
async function provisionUser(userId: string, teamId?: string) {
  const accessPoint = await efsClient.send(
  ...
  try {
    ...
    if (err.name === 'ConditionalCheckFailedException') {
      ...
      if (existingStatus === 'provisioning') {
        ...
        const newAccessPoint = await efsClient.send(
          new CreateAccessPointCommand({
            FileSystemId: process.env['EFS_FILE_SYSTEM_ID']!,
            PosixUser: { Uid: 1000, Gid: 1000 },
            RootDirectory: {
        ...
        try {
          // Use conditional write to set status 'starting' (status lock)
          try {
            await ddbClient.send(
              new PutItemCommand({
                TableName: process.env['USERS_TABLE_NAME']!,
                ...
                ConditionExpression: '#s = :provisioned OR #s = :provisioning',
            ...
            if (lockErr.name === 'ConditionalCheckFailedException') {
              // Another invocation already advanced the status
              // Clean up the access point we just created
              try {
                await efsClient.send(
                  new DeleteAccessPointCommand({
                ...
                console.warn(
                  `[lifecycle] Failed to cleanup access point ${newAccessPoint.AccessPointId}:`,
                  cleanupErr?.message,
                );
              }
              return {
                statusCode: 200,
                body: JSON.stringify({ alreadyStarted: true }),
              };
            }
            throw lockErr;
          }
          ...
          // Clean up orphaned access point
          if (newAccessPoint?.AccessPointId) {
            try {
              await efsClient.send(
                new DeleteAccessPointCommand({
                  AccessPointId: newAccessPoint.AccessPointId,
...
async function ensureUserTargetGroup(userId: string): Promise<string> {
  ...
  try {
    const desc = await elbv2Client.send(
  ...
  const tg = await elbv2Client.send(
...
async function ensureListenerRule(
  ...
  const existingRules = await elbv2Client.send(
  ...
  // Retry with incremented priority on collision
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const rule = await elbv2Client.send(
      ...
      if (err.name === 'PriorityInUseException' && attempt < 4) {
        nextPriority++;
        continue;
      }
      throw err;
  ...
  throw new Error('Failed to create listener rule after 5 priority attempts');
}

/**
 * Resolve all integration secret ARNs for a user by cascade: user > team > global.
 * Returns a map of integrationId -> { secretArn, envVarPrefix }.
 * Only ARN references are returned — no actual secret values.
 */
async function resolveAllIntegrationSecretArns(
  userId: string,
  teamId: string | undefined,
): Promise<Record<string, { secretArn: string; envVarPrefix: string }>> {
  const INTEGRATIONS_TABLE = process.env['INTEGRATIONS_TABLE_NAME'];
  const DEPLOY_ENV = process.env['DEPLOY_ENV'];
  if (!INTEGRATIONS_TABLE || !DEPLOY_ENV) {
    console.error(
      '[lifecycle] INTEGRATIONS_TABLE_NAME or DEPLOY_ENV not set — skipping credential resolution',
    );
    return {};
  }

  // Get all globally enabled integrations
  const globalItems = await ddbClient.send(
    new QueryCommand({
      TableName: INTEGRATIONS_TABLE,
      IndexName: 'by-scope',
      KeyConditionExpression: 'scopeKey = :sk',
      ExpressionAttributeValues: { ':sk': { S: 'global' } },
    }),
  );

  const result: Record<string, { secretArn: string; envVarPrefix: string }> =
    {};

  // Build a set of globally enabled integration IDs for filtering
  const globalEnabledIds = new Set<string>();
  const globalAllowOverrideMap = new Map<string, boolean>();

  for (const item of globalItems.Items || []) {
    const integrationId = item['integrationId']?.S;
    const enabled = item['enabled']?.BOOL;
    if (!integrationId || !enabled) continue;

    globalEnabledIds.add(integrationId);

    // Check allowUserOverride from global item
    const allowUserOverride = item['allowUserOverride']?.BOOL !== false;
    globalAllowOverrideMap.set(integrationId, allowUserOverride);

    // Cascade: user > team > global — pick the first secret path that should exist
    const secretPaths = [
      // Only include user-level path if overrides are allowed at the global level
      ...(allowUserOverride
        ? [`tc/integrations/${DEPLOY_ENV}/user/${userId}/${integrationId}`]
        : []),
      ...(teamId
        ? [`tc/integrations/${DEPLOY_ENV}/team/${teamId}/${integrationId}`]
        : []),
      `tc/integrations/${DEPLOY_ENV}/global/${integrationId}`,
    ];

    // We pass all candidate ARNs; generate-config.js will try them in order
    result[integrationId] = {
      secretArn: secretPaths.join(','),
      envVarPrefix: getEnvVarPrefix(integrationId),
    };
  }

  // Also check team-level allowUserOverride for each integration
  // Build a map of team-level allowUserOverride settings
  const teamAllowOverrideMap = new Map<string, boolean>();
  if (teamId) {
    const teamItems = await ddbClient.send(
      new QueryCommand({
        TableName: INTEGRATIONS_TABLE,
        IndexName: 'by-scope',
        KeyConditionExpression: 'scopeKey = :sk',
        ExpressionAttributeValues: { ':sk': { S: `team#${teamId}` } },
      }),
    );
    for (const tItem of teamItems.Items || []) {
      const tId = tItem['integrationId']?.S;
      if (tId && tItem['allowUserOverride']?.BOOL === false) {
        teamAllowOverrideMap.set(tId, false);
      }
    }
  }

  // Re-check: remove user-level paths from secretPaths if team disallows override
  for (const [integrationId, entry] of Object.entries(result)) {
    if (teamAllowOverrideMap.get(integrationId) === false) {
      const paths = entry.secretArn
        .split(',')
        .filter(p => !p.includes(`/user/${userId}/`));
      entry.secretArn = paths.join(',');
    }
  }

  // Also check user-level integrations — but only include if globally enabled
  // AND allowUserOverride is true at both global and team level
  const userItems = await ddbClient.send(
    new QueryCommand({
      TableName: INTEGRATIONS_TABLE,
      IndexName: 'by-scope',
      KeyConditionExpression: 'scopeKey = :sk',
  ...
  for (const item of userItems.Items || []) {
    const integrationId = item['integrationId']?.S;
    if (!integrationId || result[integrationId]) continue;
    // Only include user integrations where the global item has enabled: true
    if (!globalEnabledIds.has(integrationId)) continue;
    // Check allowUserOverride at global level
    if (!globalAllowOverrideMap.get(integrationId)) continue;
    // Check allowUserOverride at team level
    if (teamAllowOverrideMap.get(integrationId) === false) continue;

    result[integrationId] = {
      secretArn: `tc/integrations/${DEPLOY_ENV}/user/${userId}/${integrationId}`,
      envVarPrefix: getEnvVarPrefix(integrationId),
    };
  }
...
async function startContainer(userId: string) {
  const userRecord = await ddbClient.send(
    new GetItemCommand({
      TableName: process.env['USERS_TABLE_NAME']!,
      Key: { userId: { S: userId } },
    }),
  ...
  if (
    userRecord.Item['status']?.S === 'running' &&
    userRecord.Item['taskArn']?.S
  ) {
    ...
    if (task && task.lastStatus !== 'STOPPED') {
      return {
        statusCode: 200,
        body: JSON.stringify({ taskArn: task.taskArn, alreadyRunning: true }),
      };
    }
  }

  // Resolve integration secret ARNs before starting the container
  // Only ARN references (non-sensitive) are passed as env vars;
  // the container fetches actual credentials from Secrets Manager at runtime.
  const teamId = userRecord.Item['teamId']?.S || undefined;
  let integrationSecretArnsB64 = '';
  try {
    const resolvedArns = await resolveAllIntegrationSecretArns(userId, teamId);
    if (Object.keys(resolvedArns).length > 0) {
      integrationSecretArnsB64 = Buffer.from(
        JSON.stringify(resolvedArns),
      ).toString('base64');
    }
  } catch (err) {
    console.warn(
      'Failed to resolve integration secret ARNs, proceeding without them:',
      err,
    );
  }
        ...
        containerOverrides: [
          {
            ...
            environment: [
              ...
              { name: 'TEAM_ID', value: userRecord.Item['teamId']?.S || '' },
              {
                ...
                value: process.env['API_KEYS_SECRET_ARN']!,
              },
              {
                name: 'ALLOWED_ORIGINS',
                value: process.env['ALLOWED_ORIGINS'] || '',
              },
              ...(integrationSecretArnsB64
                ? [
                    {
                      name: 'INTEGRATION_SECRET_ARNS',
                      value: integrationSecretArnsB64,
                    },
                  ]
                : []),
            ],
          },
          {
            ...
            environment: [
              ...
              },
              {
                ...
                value: process.env['USAGE_TABLE_NAME'] || '',
              },
              { name: 'USER_ID', value: userId },
            ],
          },
  ...
  if (!taskArn) {
    ...
    return {
      statusCode: 503,
      body: JSON.stringify({ error: `ECS launch failed: ${reason}` }),
    };
  }
  ...
  const privateIp = await waitForTaskIp(taskArn);

  if (!privateIp) {
    // Check if the task stopped or we simply timed out
    const desc = await ecsClient.send(
      new DescribeTasksCommand({
        cluster: process.env['ECS_CLUSTER_NAME']!,
        tasks: [taskArn],
    ...
    const taskStatus = desc.tasks?.[0]?.lastStatus;
    const stoppedReason = desc.tasks?.[0]?.stoppedReason ?? 'unknown';
    ...
    if (taskStatus === 'STOPPED') {
      return {
        statusCode: 503,
        body: JSON.stringify({ error: `Task stopped: ${stoppedReason}` }),
      };
    }
    return {
      statusCode: 503,
      body: JSON.stringify({ error: 'Timed out waiting for task IP' }),
    };
  }

  if (process.env['ALB_LISTENER_ARN'] && process.env['VPC_ID']) {
  ...
  } else if (process.env['ALB_TARGET_GROUP_ARN']) {
  ...
  await ddbClient.send(
    new PutItemCommand({
      ...
      Item: {
        ...
        ...(userTargetGroupArn
          ? { targetGroupArn: { S: userTargetGroupArn } }
          : {}),
...
async function stopContainer(userId: string) {
  ...
  if (userTgArn && privateIp) {
    try {
      ...
      console.warn(
        `[lifecycle] Failed to deregister target ${privateIp}:`,
        err?.message,
      );
    }
    ...
    if (ruleArn) {
      try {
        ...
        console.warn(
          `[lifecycle] Failed to delete listener rule ${ruleArn}:`,
          err?.message,
        );
      }
    }
    ...
    try {
      await elbv2Client.send(
        new DeleteTargetGroupCommand({ TargetGroupArn: userTgArn }),
      );
    } catch (err: any) {
      console.warn(
        `[lifecycle] Failed to delete target group ${userTgArn}:`,
        err?.message,
      );
    }
    ...
    try {
      ...
      console.warn(
        `[lifecycle] Failed to deregister target from shared TG ${privateIp}:`,
        err?.message,
      );
    }
  }
...
async function checkIdleContainers() {
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({ checked: allItems.length, stopped }),
  };
}
