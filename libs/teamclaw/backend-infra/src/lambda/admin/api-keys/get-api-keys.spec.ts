...
describe('get-api-keys handler', () => {
  ...
  it('should return masked API keys with keyId (last 8 chars)', async () => {
    mockSend.mockResolvedValueOnce({
      SecretString: newFormatSecret({
        openai: {
          authType: 'apiKey',
          keys: ['sk-abcdefghijklmnop', 'sk-1234567890abcdef'],
        },
    ...
    expect(body.providers.openai.keys[0].keyId).toBe('ijklmnop'); // last 8 of 'sk-abcdefghijklmnop'
    expect(body.providers.openai.keys[0].masked).toMatch(/^\*+mnop$/);
    expect(body.providers.openai.keys[0]).not.toHaveProperty('index');
    expect(body.providers.openai.keys[1].keyId).toBe('90abcdef'); // last 8 of 'sk-1234567890abcdef'

    // anthropic provider
    expect(body.providers.anthropic.keys).toHaveLength(1);
    expect(body.providers.anthropic.keys[0].keyId).toBe('key12345'); // last 8 of 'ant-key12345'
  });

  it('should return keyId as full key when key is 8 chars or shorter', async () => {
    mockSend.mockResolvedValueOnce({
      SecretString: newFormatSecret({
        test: { authType: 'apiKey', keys: ['abcd1234'] },
      }),
    });
    ...
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    // 8-char key: slice(-8) returns the whole key
    expect(body.providers.test.keys[0].keyId).toBe('abcd1234');
  });

  it('should return keyId for short keys (< 8 chars)', async () => {
    mockSend.mockResolvedValueOnce({
      SecretString: newFormatSecret({
        test: { authType: 'apiKey', keys: ['abc'] },
      }),
    });
    ...
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    // 3-char key: slice(-8) returns the whole key
    expect(body.providers.test.keys[0].keyId).toBe('abc');
  });
  ...
  it('should return OAuth provider without expiresAt when absent', async () => {
    mockSend.mockResolvedValueOnce({
      SecretString: newFormatSecret({
        google: {
          authType: 'oauthToken',
  ...
  it('should return mixed providers (apiKey and oauthToken)', async () => {
    mockSend.mockResolvedValueOnce({
      SecretString: newFormatSecret({
        openai: { authType: 'apiKey', keys: ['sk-abcdefghijklmnop'] },
    ...
    const body = JSON.parse(res.body);
    expect(body.providers.openai.authType).toBe('apiKey');
    expect(body.providers.openai.keys[0].keyId).toBe('ijklmnop');
    expect(body.providers.google.authType).toBe('oauthToken');
    expect(body.providers.google.hasToken).toBe(true);
  });
  ...
  it('should handle empty keys array', async () => {
    ...
    const res = await invoke();
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).providers.openai.keys).toEqual([]);
  });
