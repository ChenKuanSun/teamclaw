...
describe('AdminCognitoStack', () => {
  ...
  test('creates UserPoolClient with SRP auth only (no client secret)', () => {
    ...
    for (const key of Object.keys(clients)) {
      const authFlows =
        (clients[key] as any).Properties?.ExplicitAuthFlows ?? [];
  ...
  test('DEV env adds localhost callback URLs', () => {
    devTemplate.hasResourceProperties('AWS::Cognito::UserPoolClient', {
      CallbackURLs: Match.arrayWith(['http://localhost:4900/auth/callback']),
      LogoutURLs: Match.arrayWith(['http://localhost:4900/auth/login']),
    });
  });
