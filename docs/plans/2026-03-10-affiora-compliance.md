...
### Apps
- `apps/web-admin/` — Angular 21 admin panel
- `apps/web-chat/` — Angular 21 chat interface
- `apps/infra-foundation/` — CDK: VPC, EFS, ECR
- `apps/infra-cluster/` — CDK: ECS Cluster, ALB
- `apps/infra-control-plane/` — CDK: Cognito, DynamoDB, Lambda
- `apps/infra-admin/` — CDK: Admin API Gateway, Lambda, Amplify
...
### Task 13: Add `withComponentInputBinding()` to web-chat (I-7)

**Files:**
- Modify: `apps/web-chat/src/app/app.config.ts`
...
```bash
git add apps/web-chat/src/app/app.config.ts
git commit -m "feat(web-chat): add withComponentInputBinding for route param binding"
```
