...
**Architecture:** Separate Angular 21 SPA (`enterprise-admin`) with its own Cognito User Pool (PKCE OAuth), REST API Gateway + WAF, and Lambda functions. Deployed via Amplify. All CDK patterns mirror Affiora's admin stacks: SSM cross-stack refs, `StackPropsWithEnv`, separate CDK app for admin infra.
...
|---|---|
| `apps/30-secretary/secretary-admin-app/` | `apps/enterprise-admin/` |
| `apps/30-secretary/secretary-backend-admin-infra/` | `apps/30-admin/teamclaw-admin-infra/` |
...
- Separate User Pool from enterprise-chat's Cognito (prevents password conflicts)
...
**Files:**
- Create: `apps/30-admin/teamclaw-admin-infra/cdk/app.ts`
- Create: `apps/30-admin/teamclaw-admin-infra/cdk/main.ts`
- Create: `apps/30-admin/teamclaw-admin-infra/cdk.json`
- Create: `apps/30-admin/teamclaw-admin-infra/tsconfig.json`
- Create: `apps/30-admin/teamclaw-admin-infra/project.json`
...
Copy from existing `apps/20-control-plane/teamclaw-control-plane-infra/` and adjust paths.

**Step 4: Commit**

```bash
git add apps/30-admin/teamclaw-admin-infra/
...
**Files:**
- Create: `apps/enterprise-admin/project.json`
- Create: `apps/enterprise-admin/tsconfig.json`
- Create: `apps/enterprise-admin/tsconfig.app.json`
- Create: `apps/enterprise-admin/tsconfig.spec.json`
- Create: `apps/enterprise-admin/jest.config.ts`
- Create: `apps/enterprise-admin/src/main.ts`
- Create: `apps/enterprise-admin/src/index.html`
- Create: `apps/enterprise-admin/src/styles.scss`
- Create: `apps/enterprise-admin/src/test-setup.ts`
- Create: `apps/enterprise-admin/src/environments/environment.ts`
- Create: `apps/enterprise-admin/src/environments/environment.development.ts`
...
- Serve target on port 4900 (different from enterprise-chat's port)
...
Follow enterprise-chat's index.html (includes `global` polyfill since we may use Cognito JS SDK):
...
```bash
git add apps/enterprise-admin/
git commit -m "feat(admin): scaffold enterprise-admin Angular app"
```
...
**Files:**
- Create: `apps/enterprise-admin/src/app/services/admin-auth.service.ts`
- Create: `apps/enterprise-admin/src/app/services/admin-api.service.ts`
- Create: `apps/enterprise-admin/src/app/guards/auth.guard.ts`
- Create: `apps/enterprise-admin/src/app/guards/auth.interceptor.ts`
...
```bash
git add apps/enterprise-admin/src/app/services/ apps/enterprise-admin/src/app/guards/
...
**Files:**
- Create: `apps/enterprise-admin/src/app/app.component.ts`
- Create: `apps/enterprise-admin/src/app/app.config.ts`
- Create: `apps/enterprise-admin/src/app/app.routes.ts`
- Create: `apps/enterprise-admin/src/app/auth/auth.routes.ts`
- Create: `apps/enterprise-admin/src/app/auth/login/login.component.ts`
- Create: `apps/enterprise-admin/src/app/auth/callback/callback.component.ts`
- Create: `apps/enterprise-admin/src/app/layout/layout.component.ts`
...
```bash
git add apps/enterprise-admin/src/app/
...
**Files:**
- Create: `apps/enterprise-admin/src/app/features/dashboard/dashboard.component.ts`
...
```bash
git add apps/enterprise-admin/src/app/features/dashboard/
...
**Files:**
- Create: `apps/enterprise-admin/src/app/features/users/users.component.ts`
- Create: `apps/enterprise-admin/src/app/features/user-detail/user-detail.component.ts`
...
```bash
git add apps/enterprise-admin/src/app/features/users/ apps/enterprise-admin/src/app/features/user-detail/
...
**Files:**
- Create: `apps/enterprise-admin/src/app/features/teams/teams.component.ts`
- Create: `apps/enterprise-admin/src/app/features/team-detail/team-detail.component.ts`
- Create: `apps/enterprise-admin/src/app/features/containers/containers.component.ts`
...
```bash
git add apps/enterprise-admin/src/app/features/teams/ apps/enterprise-admin/src/app/features/team-detail/ apps/enterprise-admin/src/app/features/containers/
...
**Files:**
- Create: `apps/enterprise-admin/src/app/features/config/config.component.ts`
- Create: `apps/enterprise-admin/src/app/features/api-keys/api-keys.component.ts`
- Create: `apps/enterprise-admin/src/app/features/analytics/analytics.component.ts`
...
```bash
git add apps/enterprise-admin/src/app/features/config/ apps/enterprise-admin/src/app/features/api-keys/ apps/enterprise-admin/src/app/features/analytics/
...
- Modify: `libs/teamclaw/backend-infra/src/index.ts`
- Modify: `apps/30-admin/teamclaw-admin-infra/cdk/app.ts`
        ...
        build:
          commands:
            - if [ "${AWS_BRANCH}" = "main" ]; then npx nx run
              enterprise-admin:build:production --verbose; fi
            - if [ "${AWS_BRANCH}" = "dev" ]; then npx nx run
              enterprise-admin:build:development --verbose; fi
      artifacts:
        baseDirectory: ../../dist/apps/enterprise-admin/browser/
        files:
          - '**/*'
      cache:
        paths:
...
```typescript
// In apps/30-admin/teamclaw-admin-infra/cdk/app.ts, add:
...
```bash
git add libs/teamclaw/backend-infra/src/stack/admin-amplify.stack.ts amplify-admin.yml apps/30-admin/teamclaw-admin-infra/cdk/app.ts libs/teamclaw/backend-infra/src/index.ts
...
**Files:**
- Modify: `apps/enterprise-chat/project.json` (line 69: `jest.config.cts` → `jest.config.ts`)

**Step 1: Fix jest config reference**

```json
"jestConfig": "apps/enterprise-chat/jest.config.ts"
```

**Step 2: Commit**

```bash
git add apps/enterprise-chat/project.json
git commit -m "fix: correct jest.config.ts reference in enterprise-chat project.json"
```
...
cd apps/30-admin/teamclaw-admin-infra
