...
**Files:**
- Modify: `apps/web-admin/src/app/features/dashboard/onboarding-wizard.component.ts`
...
Check `apps/web-admin/src/app/services/admin-api.service.ts` — ensure `addApiKey` accepts a generic object (not just `{ provider, key }`). If it's typed narrowly, widen it:
...
Run: `npx nx test web-admin --no-coverage`
Expected: PASS

**Step 6: Commit**

```bash
git add apps/web-admin/src/app/features/dashboard/onboarding-wizard.component.ts apps/web-admin/src/app/services/admin-api.service.ts
